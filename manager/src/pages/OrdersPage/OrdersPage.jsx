...
const OrdersPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Remove orderType from API query to fetch all orders
  ...
  const filteredOrders = orders.filter(order => {
    return (
      ...
      (
        !filters.orderType ||
        (filters.orderType === "dineIn" && order.orderType === "dine-in") ||
        (filters.orderType === "takeaway" && order.orderType === "takeaway") ||
        (filters.orderType === "delivery" && order.orderType === "delivery")
      )
    );
  });
      ...
      <div className="w-6xl mx-3 px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          ...
          <div className="mt-4 md:mt-0 flex space-x-3">
            ...
            >
              <svg className="w-5 h-5 mr-2 -ml-1" fill="none" stroke="white" viewBox="0 0 24 24">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">In-Restaurant</p>
                <p className="text-2xl font-semibold text-gray-900">
                  {orders.filter(o => o.orderType === "dine-in" || o.orderType === "takeaway").length}
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">Home Delivery</p>
                <p className="text-2xl font-semibold text-gray-900">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full px-4 py-2 bg-indigo-600 text-black rounded-lg hover:bg-indigo-700 transition-colors font-medium flex items-center justify-center"
              >
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOrders.length > 0 ? (
                      ...
                      {filters.orderType !== "delivery" && (
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            order.orderType === "dine-in" ? "bg-purple-100 text-purple-800" : "bg-blue-100 text-blue-800"
                          }`}>
                            {order.orderType}
                          </span>
                          {order.tableNumber && (
                            <div className="text-xs text-gray-500 mt-1">Table #{order.tableNumber}</div>
                          )}
                        </td>
                      )}
                      {filters.orderType === "delivery" && (
                  ...
                  <tr>
                    <td colSpan={filters.orderType !== "delivery" ? 7 : 6} className="px-6 py-12 text-center">
